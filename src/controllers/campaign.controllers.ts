...
function getCampaignOpenStatuses(status: string) {
...
export async function listCampaignApplications(req: Request, res: Response) {
  try {
    const user = req.user;
    if (!user) return res.status(401).json({ error: "UNAUTHORIZED" });
    // Ignore TS role typing mismatch; runtime checks based on spec roles.
    if (
      !((user as any).role === "BUSINESS" || (user as any).role === "ADMIN")
    ) {
      return res.status(403).json({ error: "INSUFFICIENT_ROLE" });
    }
...
export async function patchCampaignApplication(req: Request, res: Response) {
  try {
    const user = req.user;
    if (!user) return res.status(401).json({ error: "UNAUTHORIZED" });
    if (!((user as any).role === "BUSINESS")) {
